...
def sendMailAprendices():
    ...
    sqlQuery2 = f"""SELECT * FROM EvalFechas"""
    aprendices = call_db(sqlQuery1)
    evalFechas = call_db(sqlQuery2)

    dates = {'endInstPhoto':evalFechas[0][2], 'endEvaluation':evalFechas[0][3]}

    for i in coordinaciones:
        mailData = {'endInstPhoto':evalFechas[0][2], 'endEvaluation':evalFechas[0][3],
                    'NOMBRE': i[2], 'APELLIDOS': i[3], 'CORREO_ELECTRONICO': i[5], 'FICHA': I[8], 'HASH': i[9],
                    'subject': "Activacion del Aplicativo para Evaluar a los Instructores"}
        ...
        La plataforma para que los aprendices den apreciaciones sobre su estilo de enseñanza ha sido activada y le solicitamos muy amablemente ingrese a la plataforma para dar sus apreciaciones.
